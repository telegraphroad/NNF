...
for i in range(num_layers):
    s = nf.nets.MLP([latent_size, lw * latent_size,lw * latent_size, latent_size], init_zeros=True)
    t = nf.nets.MLP([latent_size, lw * latent_size,lw * latent_size, latent_size], init_zeros=True)
    if i % 2 == 0:
...
#traceback.print_exc()                        
del a,checkpoints
torch.save(bestmodel,f'zbestmodeldouble_{nm}_{lw}_{num_layers}.ph')
torch.save(model,f'zmodeldouble_{nm}_{lw}_{num_layers}.ph')
# import pandas as pd
# cat_cols = []
# bestmodel.cpu()
# model.cpu()
