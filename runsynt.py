...
nfm = nf.NormalizingFlow    (q0=q0, flows=flows,categoricals=None,catlevels=None,catvdeqs=None)
nfmBest = nf.NormalizingFlow(q0=q0, flows=flows,categoricals=None,catlevels=None,catvdeqs=None)
if targetD == 'Toy':
    ...
    xcol = s.columns
    X = s.values
elif targetD == 'CircularGaussian8':
    td = nf.distributions.target.CircularGaussianMixture(n_modes=8)
elif targetD == 'CircularGaussian3':
    td = nf.distributions.target.CircularGaussianMixture(n_modes=3)
elif targetD == 'StudentT':
    td = nf.distributions.target.StudentTDist(dim=latent_size,df=prm)
elif targetD == 'NealsFunnel':    
    td = nf.distributions.target.NealsFunnel(v1shift=prm)
elif targetD == 'RingMixture3':        
    td = nf.distributions.target.RingMixture(n_rings=3)
elif targetD == 'RingMixture8':        
    td = nf.distributions.target.RingMixture(n_rings=3)
elif targetD == 'TwoMoons':        
    td = nf.distributions.target.TwoMoons()


if targetD != 'Toy':
    X = td.sample(30000).cpu().detach().numpy()
...
nfm.train()
dgrads.append([(n,p.grad) for n,p in nfm.flows[0].named_parameters()])
...
torch.save(dgrads, f'/home/samiri/NNF/logs/gradsdet_targetD_{targetD}_nc_{nc}_cb_{cb}_trainable_{tparam}_nunit_{nu}_param_{prm}_base_{based}.pth')
